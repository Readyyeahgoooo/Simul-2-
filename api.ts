...
export default async function handler(req: any, res: any) {
  ...
  const chosenModel =
    typeof model === "string" && model.trim() ? model : DEFAULT_MODEL;
  ...
  try {
    const r = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        // Optional; only include if you actually want reasoning fields
    ...
    // Your app expects JSON. Try to parse.
    let parsed: any = null;
    try {
    ...
    if (!parsed) {
      // Return safe fallback so UI won't crash
      res.status(200).json({ event: content || "Model returned empty content." });
      return;
    }
